...
import {Typography,Container,Box,Grid,Checkbox,FormControlLabel,TextField,CssBaseline,Button,Avatar, Snackbar,Alert} from '@mui/material';
...
useEffect(()=>{
	async function getProfile(){
		try {
			const response = await axios.get(`http://localhost:8000/api/profile/${GlobalState.userId}/`)
...
useEffect(() => {
	if (state.sendRequest) {
		async function AddProperty() {
			...
			formData.append("tittle", state.titleValue);
			...
			if (state.latitudeValue) formData.append("latitude", state.latitudeValue);
			if (state.longitudeValue) formData.append("longitude", state.longitudeValue);
			...
			try {
				const response = await axios.post("http://localhost:8000/api/listing/create/", formData, {
					headers: {
            ...
            <Grid item container justifyContent="space-between">
                ...
                >
                    ...
                    >
						...
						<input  hidden type='file' multiple accept = "image/png, image/gif, image/jpeg" />
                    </Button>
                </Grid>
