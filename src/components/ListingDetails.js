		...
		async function GetListingInfo() {
			try {
				const response = await Axios.get(
					`http://localhost:8000/api/listing/${params.id}/`
				);
    ...
    useEffect(() => {
		if (state.listingInfo) {
			async function GetProfileInfo() {
				try {
					const response = await Axios.get(
						`http://localhost:8000/api/profile/${state.listingInfo.seller}/`
					);
		...
		if (confirmDelete) {
			try {
				const response = await Axios.delete(
					`http://localhost:8000/api/listing/${params.id}/delete/`
				);
    ...
    {listingPictures.length > 0 ? (
			...
			>
				<Grid item container xs={7} direction="column" spacing={1}>
					<Grid item>
						<Typography variant="h5">{state.listingInfo.tittle}</Typography>
					</Grid>
					<Grid item>
						<RoomIcon />{" "}
